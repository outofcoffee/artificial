...
- `outfit.go` — the `Outfit` file format: a flat, Dockerfile-style description of one provider selection. `parseOutfit` reads it into a `selection` (keywords case-insensitive, UPPERCASE canonical, `#` comments); `formatOutfit` renders one back out for `export`. `apply` defaults to `./Outfit` (`DefaultOutfitFile`).
